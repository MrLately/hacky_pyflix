...
async def get_stream(type, id):
    ...
    season_episode_format = f"S{int(season):02d}E{int(episode):02d}" if season and episode else None
    ...
    try:
        with open('unrestricted_links.txt', 'r') as file:
            for line in file:
                parts = line.strip().split(' | ')
                if len(parts) == 4:
                    catalog_type, link_video_id, se_ep_info, link = parts
                elif len(parts) == 3:
                    catalog_type, link_video_id, link = parts
                    se_ep_info = None
                else:
                    continue

                link_video_id = link_video_id.replace("Video ID: ", "").strip()
              
                if catalog_type == 'Series' and se_ep_info and season_episode_format:
                    if video_id == link_video_id and season_episode_format == se_ep_info:
                        continue_streams.append({"name": "PyFlix ⚡🔄", "title": link.split('/')[-1], "url": link})
                
                elif catalog_type == 'Movie' and type == 'movie' and video_id == link_video_id:
                    continue_streams.append({"name": "PyFlix ⚡🔄", "title": link.split('/')[-1], "url": link})
    except FileNotFoundError:
        pass

    streams = continue_streams + [
        {
